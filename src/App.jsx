...
// --- Storage helpers (in-memory fallback since no localStorage) ---
const memStore = {};
async function stGet(key) { return memStore[key] || null; }
async function stSet(key, val) { memStore[key] = val; }
async function stGetAll(prefix) {
  return Object.entries(memStore).filter(([k])=>k.startsWith(prefix)).map(([k,v])=>({...v,_key:k}));
}
...
.tab-btn{padding:6px 14px;border-radius:8px;border:1px solid #e0e0e0;background:#f5f5f5;color:#666;cursor:pointer;font-size:12px;}
          ...
          {analysis&&<div>
            ...
            <div style={{background:"#e5e7eb",borderRadius:16,padding:16,overflowY:"auto",maxHeight:700}}>
              <div ref={reportRef} style={{background:"#fff",fontFamily:"'Malgun Gothic',sans-serif"}}>
                <div style={{width:"100%",padding:"40px 36px",boxSizing:"border-box"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:24,paddingBottom:16,borderBottom:"3px solid #4f46e5"}}>
                    <div><h1 style={{fontSize:28,fontWeight:700,color:"#312e81",marginBottom:4}}>시험분석 보고서</h1><span style={{background:"#4f46e5",color:"#fff",fontSize:10,padding:"2px 8px",borderRadius:4,fontWeight:700}}>Analysis Report</span></div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,background:"#f8fafc",padding:20,borderRadius:12,marginBottom:20,border:"1px solid #e2e8f0"}}>
                    {[["👤 이름/학년",`${info.studentName} (${info.grade})`],["📚 과목/성적",`${info.subject} (${info.score}점)`],["🏫 학교",info.schoolName],["📋 시험범위",info.examRange]].map(([l,v])=>(
                      <div key={l} style={{display:"flex",gap:10,alignItems:"center"}}>
                        ...
                        <div><p style={{fontSize:9,color:"#94a3b8",fontWeight:700,marginBottom:2}}>{l.split(" ").slice(1).join(" ")}</p><p style={{fontSize:13,fontWeight:700,color:"#1e293b"}}>{v}</p></div>
                      </div>
                    ))}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:16,marginBottom:20}}>
                    <div>
                      ...
                      {[["상",analysis.difficultyDistribution?.high||0,"#ef4444"],["중",analysis.difficultyDistribution?.medium||0,"#f59e0b"],["하",analysis.difficultyDistribution?.low||0,"#10b981"]].map(([l,v,c])=>(
                        <div key={l} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                          ...
                          <div style={{flex:1,background:"#f1f5f9",borderRadius:20,height:10}}><div style={{width:`${v}%`,height:"100%",background:c,borderRadius:20}}/></div>
                    ...
                    <div>
                      ...
                      <div style={{background:"#f8fafc",padding:14,borderRadius:10,border:"1px solid #e2e8f0",fontSize:11,color:"#475569",lineHeight:1.7}}>{analysis.trends}</div>
                    </div>
                  </div>
                  <p style={{fontSize:13,fontWeight:700,color:"#312e81",marginBottom:10}}>✅ 문항별 분석</p>
                  <div style={{border:"1px solid #e2e8f0",borderRadius:10,overflow:"hidden",marginBottom:20}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                      <thead><tr style={{background:"#f8fafc",borderBottom:"1px solid #e2e8f0"}}>{["No","주요 개념","난이도","결과","코멘트"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:700,color:"#64748b",fontSize:10}}>{h}</th>)}</tr></thead>
                      <tbody>{analysis.questionAnalysis?.map((q,i)=>(
                        <tr key={i} style={{borderBottom:"1px solid #f1f5f9"}}>
                          ...
                          <td style={{padding:"6px 10px"}}><span style={{padding:"2px 6px",borderRadius:4,fontSize:10,fontWeight:700,background:q.difficulty==="상"?"#fee2e2":q.difficulty==="중"?"#fef3c7":"#d1fae5",color:q.difficulty==="상"?"#b91c1c":q.difficulty==="중"?"#92400e":"#065f46"}}>{q.difficulty}</span></td>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
                    <div style={{background:"#ecfdf5",padding:20,borderRadius:12,border:"1px solid #a7f3d0"}}><p style={{fontSize:13,fontWeight:700,color:"#065f46",marginBottom:10}}>✅ 학습 성취 및 강점</p><p style={{fontSize:11,color:"#334155",lineHeight:1.7}}>{analysis.strengths}</p></div>
                    <div style={{background:"#fff1f2",padding:20,borderRadius:12,border:"1px solid #fecdd3"}}><p style={{fontSize:13,fontWeight:700,color:"#9f1239",marginBottom:10}}>⚠️ 취약 단원 및 보완점</p><p style={{fontSize:11,color:"#334155",lineHeight:1.7}}>{analysis.weaknesses}</p></div>
                  </div>
...
function ProgressTab({allChecks}){
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{background:"#FFFDF0",borderRadius:10,padding:14,border:"1px solid #FFE082"}}>
            ...
            <span style={lb}>수업 안내 이미지</span>
            <div onClick={()=>fileRef.current?.click()} style={{border:"2px dashed #ddd",borderRadius:8,padding:16,textAlign:"center",cursor:"pointer",background:"#fafafa"}}>
        ...
        <div>
          ...
          <div style={{background:"#e5e7eb",borderRadius:16,padding:16}}>
            <div style={{background:"#fff",padding:"32px 28px",position:"relative",overflow:"hidden",minHeight:400}}>
              ...
              <div style={{textAlign:"center",paddingTop:12}}><p style={{fontSize:14,fontWeight:700,color:"#1a1a1a"}}>{form.teacherName?`${form.teacherName} 코치`:"코치"} 드림</p></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="card">
      ...
      {!preview?(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div style={{background:"#f0f7ff",borderRadius:10,padding:"12px 16px",border:"1px solid #BBDEFB",display:"flex",alignItems:"center",justifyContent:"space-between",gridColumn:"1/-1"}}>
              <div>
                <p style={{fontSize:11,color:"#1565C0",fontWeight:600,marginBottom:2}}>📊 예상 월 수업료</p>
        ...
        <div>
          ...
          <div style={{background:"#fff",padding:"40px 36px",fontFamily:"'Malgun Gothic','Apple SD Gothic Neo',sans-serif",maxWidth:700,margin:"0 auto",border:"1px solid #ddd",borderRadius:8}}>
            <div style={{textAlign:"center",marginBottom:32}}>
              <h1 style={{fontSize:26,fontWeight:700,color:"#1a1a1a",marginBottom:6}}>수업 일정 및 로그인 안내</h1>
              <p style={{fontSize:13,color:"#888"}}>수업 일정 및 수업료, 로그인 정보를 확인해주세요.</p>
            </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:10,paddingLeft:20}}>
                  {[["수업 대상",form.grade||"—"],["수업 횟수",`주 ${form.times}회`]].map(([k,v])=>(
                    <div key={k} style={{display:"flex",gap:12,fontSize:13}}><span style={{color:"#888",minWidth:72,flexShrink:0}}>{k}</span><span style={{color:"#1a1a1a"}}>{v}</span></div>
                  ))}
            ...
            <div style={{border:"1.5px solid #4A90D9",borderRadius:10,padding:"20px 24px",marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><span style={{fontSize:18}}>🛡</span><h2 style={{fontSize:16,fontWeight:700}}>학생 로그인 정보</h2></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
                <div style={{padding:"12px 16px",background:"#f9f9f9",borderRadius:8,textAlign:"center"}}><p style={{fontSize:11,color:"#888",marginBottom:6}}>홈페이지</p><p style={{fontSize:11,fontWeight:600}}>www.sscoaching.co.kr</p></div>
            ...
            <div style={{width:"100%",height:8,background:"linear-gradient(90deg,#1565C0,#42A5F5)",borderRadius:4,marginBottom:24}}/>
            <div style={{textAlign:"center",marginBottom:20}}><h2 style={{fontSize:20,fontWeight:700}}>학부모님께 드리는 말씀</h2></div>
            {messages.map((item,i)=>(
...
export default function App(){
  ...
  function login(){
    ...
    // load from memStore
    const ck = memStore["checks_"+n] || {};
    setChecks(ck);
    if(n===ADMIN){
      const ac={};
      COACHES.filter(c=>c!==ADMIN).forEach(c=>{ac[c]=memStore["checks_"+c]||{};});
      setAllChecks(ac);
    }
    setUser(n);
  }
  ...
  async function toggleCheck(id){
    ...
    setChecks(next);
    memStore["checks_"+user]=next;
    if(user===ADMIN){
      const ac={...allChecks,[user]:next};setAllChecks(ac);
    }
  }

  function addEvent(){
    if(!newEv.title||!newEv.date)return;
    const ev={...newEv,id:Date.now().toString()};
    setEvents(v=>[...v,ev]);
    memStore["events"]=[...(memStore["events"]||[]),ev];
    setNewEv({title:"",date:"",desc:""});setShowEv(false);
  }
  function delEvent(id){
    const next=events.filter(e=>e.id!==id);setEvents(next);memStore["events"]=next;setSelDay(null);
  }
  function addNotice(){
    if(!newNt.title)return;
    const n={...newNt,id:Date.now().toString(),createdAt:Date.now(),date:new Date().toLocaleDateString("ko-KR")};
    setNotices(v=>[n,...v]);memStore["notices"]=[n,...(memStore["notices"]||[])];
    setNewNt({title:"",content:""});setShowNt(false);
  }
  function delNotice(id){const next=notices.filter(n=>n.id!==id);setNotices(next);memStore["notices"]=next;}
  ...
  const TABS=[["calendar","📅 일정"],["resource","📂 자료"],["settle","💰 정착지원금"],["notice","📌 공지"],["notice_gen","📄 안내문"],["free_lesson","🆓 무료수업"],["exam_analysis","📊 시험분석"],...(isAdmin?[["progress","👥 현황"]]:[])]
    ...
    <>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f0f2ff"}}>
        <div style={{background:"#fff",borderRadius:16,border:"1px solid #e0e0e0",padding:"2.5rem 2rem",width:320,textAlign:"center",boxShadow:"0 4px 24px #5C6BC022"}}>
          ...
          <p style={{fontSize:11,color:"#aaa",marginTop:12}}>등록된 코치: {COACHES.join(", ")}</p>
        </div>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="portal">
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1rem"}}>
          ...
          <button onClick={()=>{setUser(null);setChecks({});setTab("calendar");}} style={{fontSize:12,padding:"6px 12px",borderRadius:8,cursor:"pointer",background:"#f5f5f5",border:"1px solid #ddd",color:"#666"}}>로그아웃</button>
        </div>
        ...
        {tab==="progress"&&isAdmin&&<ProgressTab allChecks={allChecks}/>}
