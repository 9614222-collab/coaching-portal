...
const COACHES_DEFAULT = ["김윤정","임서영","윤민정","나지수","서예린","김혜영"];
      ...
      {!preview?(
        ...
        <div>
          ...
          <div ref={p1} style={{background:"#fff",padding:"32px 28px",fontFamily:"sans-serif",maxWidth:680,margin:"0 auto",border:"1px solid #ddd",borderRadius:8,marginBottom:16}}>
            <div style={{textAlign:"center",marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700,marginBottom:4}}>수업 일정 및 로그인 안내</h1></div>
          ...
          <div ref={p2} style={{background:"#fff",padding:"32px 28px",fontFamily:"sans-serif",maxWidth:680,margin:"0 auto 16px",border:"1px solid #ddd",borderRadius:8}}>
            <div style={{textAlign:"center",marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700,marginBottom:4}}>학부모님께 드리는 말씀</h1></div>
            {messages.map((item,i)=>(
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(d&&d.list&&d.list.length>0) setCoaches(d.list);
      const evs=await fbGetAll("events");
      setEvents(evs.sort((a,b)=>a.date>b.date?1:-1));
      const nts=await fbGetAll("notices");
      setNotices(nts.sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)));
    })();
  },[]);
  ...
  async function addEvent(){if(!newEv.title||!newEv.date)return;const id=await fbAdd("events",{...newEv,createdAt:Date.now()});if(id)setEvents(v=>[...v,{...newEv,id}].sort((a,b)=>a.date>b.date?1:-1));setNewEv({title:"",date:"",desc:""});setShowEv(false);}
            ...
            <div style={{overflowX:"auto"}}>
              <table className="settle-table">
                ...
                <tbody>
                  ...
                  <tr><td rowSpan={5} style={{background:"#F3E5F5",color:"#6A1B9A"}}>300,000원</td><td rowSpan={5} style={{fontSize:10,color:"#888"}}>× 계약이행일수÷전체일수</td><td>0T초과~10T미만</td><td style={{color:"#D32F2F",fontWeight:600}}>+100,000원</td></tr>
