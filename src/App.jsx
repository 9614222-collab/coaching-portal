...
const DEFAULT_MESSAGES = [
  ...
  {title:"수업 변경을 원하실 경우 최소 하루 전날 말씀해주셔야합니다.",desc:"사전 논의 없이 당일 결석하는 경우 무단결석으로 처리되어 보강수업 진행이 어려우며, 1회 수업이 완료한 것으로 간주되니 불이익이 없도록 유의해 주시기 바랍니다."},
  {title:"수업 횟수는 반드시 맞춰드립니다.",desc:"학생 및 선생님의 개인 사정으로 수업이 이루어지지 않을 경우, 남은 수업 횟수를 맞추기 위한 보강 수업이 진행되며, 모든 일정은 협의를 통해 조정됩니다."},
...
{"examLevel":"시험 난이도 분석","trends":"출제 경향 분석","difficultyDistribution":{"high":30,"medium":50,"low":20},"questionAnalysis":[{"number":"1","topic":"개념명","difficulty":"상","isCorrect":true,"comment":"코멘트"}],"strengths":"강점 분석","weaknesses":"약점 분석","curriculum":"커리큘럼 계획","coachComments":"코치 코멘트"}`;
  ...
  async function downloadImg(){
    ...
    try{
      ...
      const a=document.createElement("a");a.href=canvas.toDataURL("image/png");a.download=`${info.studentName||"학생"}_시험분석보고서.png`;a.click();
    ...
    <div className="card">
      ...
      <div style={{display:"grid",gridTemplateColumns:"320px 1fr",gap:20,alignItems:"start"}}>
        <div style={{background:"#f8fafc",borderRadius:12,padding:16,border:"1px solid #e2e8f0"}}>
          <p style={{fontSize:13,fontWeight:600,color:"#4f46e5",marginBottom:12}}>📋 회원 정보 입력</p>
          ...
          <label><span style={lb}>학교</span><input style={iSt} value={info.schoolName} onChange={e=>si("schoolName",e.target.value)} placeholder="대치중학교"/></label>
          ...
          <label><span style={lb}>틀린 문제</span><input style={iSt} value={info.wrongQuestions} onChange={e=>si("wrongQuestions",e.target.value)} placeholder="5,12,18,20"/></label>
          <span style={lb}>시험지 이미지 (최대 20개)</span>
          <div onClick={()=>fileRef.current?.click()} style={{border:"2px dashed #c7d2fe",borderRadius:8,padding:10,textAlign:"center",cursor:"pointer",background:"#eef2ff",marginBottom:8}}>
            <p style={{fontSize:12,color:"#6366f1"}}>⬆ 이미지 추가 ({info.examPaperBase64s.length}/20)</p>
          ...
          {info.examPaperBase64s.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4,marginBottom:8}}>
            {info.examPaperBase64s.map((img,i)=>(
              <div key={i} style={{position:"relative",aspectRatio:"1",borderRadius:6,overflow:"hidden",border:"1px solid #ddd"}}>
                ...
                <button onClick={()=>setInfo(p=>({...p,examPaperBase64s:p.examPaperBase64s.filter((_,j)=>j!==i)}))} style={{position:"absolute",top:2,right:2,background:"rgba(0,0,0,0.5)",color:"#fff",border:"none",borderRadius:"50%",width:16,height:16,fontSize:10,cursor:"pointer"}}>×</button>
              </div>
            ))}
          </div>}
          {error&&<p style={{fontSize:12,color:"#ef4444",marginBottom:8,padding:"8px 10px",background:"#fff1f2",borderRadius:6}}>{error}</p>}
          <button onClick={analyze} disabled={loading||!info.studentName||!info.subject} style={{width:"100%",padding:"10px",borderRadius:10,background:loading?"#c7d2fe":"#4f46e5",color:"#fff",border:"none",cursor:loading?"not-allowed":"pointer",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
        ...
        <div>
          {!analysis&&!loading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:300,color:"#aaa",fontSize:14}}><div style={{fontSize:48,marginBottom:12}}>📄</div><p>정보를 입력하고 분석을 시작하세요.</p></div>}
          {loading&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:300,color:"#6366f1",gap:16}}><div style={{width:40,height:40,border:"4px solid #c7d2fe",borderTopColor:"#4f46e5",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/><p>분석 중...</p></div>}
          {analysis&&<div>
            <div style={{display:"flex",gap:8,marginBottom:12,justifyContent:"flex-end"}}>
              <button onClick={()=>setIsEditing(!isEditing)} style={{padding:"7px 14px",borderRadius:8,background:isEditing?"#059669":"#f5f5f5",color:isEditing?"#fff":"#555",border:"1px solid #ddd",cursor:"pointer",fontSize:12,fontWeight:600}}>{isEditing?"✓ 수정 완료":"✏️ 수정하기"}</button>
            ...
            <div style={{background:"#e5e7eb",borderRadius:16,padding:16,overflowY:"auto",maxHeight:600}}>
              <div ref={reportRef} style={{background:"#fff",fontFamily:"sans-serif"}}>
                <div style={{padding:"32px 28px"}}>
                  <div style={{height:4,background:"#4f46e5",marginBottom:20}}/>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:20,paddingBottom:12,borderBottom:"2px solid #4f46e5"}}>
                    <div><h1 style={{fontSize:22,fontWeight:700,color:"#312e81"}}>시험분석 보고서</h1><span style={{background:"#4f46e5",color:"#fff",fontSize:10,padding:"2px 8px",borderRadius:4}}>PAGE 1/2</span></div>
                    <p style={{fontSize:11,color:"#94a3b8"}}>{new Date().toLocaleDateString()}</p>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,background:"#f8fafc",padding:16,borderRadius:10,marginBottom:16,border:"1px solid #e2e8f0"}}>
                    {[["👤 이름/학년",`${info.studentName} (${info.grade})`],["📚 과목/성적",`${info.subject} (${info.score}점)`],["🏫 학교",info.schoolName],["📋 시험범위",info.examRange]].map(([l,v])=>(
                      <div key={l} style={{display:"flex",gap:8,alignItems:"center"}}>
                        <div style={{background:"#e0e7ff",padding:"4px 6px",borderRadius:6,fontSize:12}}>{l.split(" ")[0]}</div>
                        <div><p style={{fontSize:9,color:"#94a3b8",fontWeight:700}}>{l.split(" ").slice(1).join(" ")}</p><p style={{fontSize:12,fontWeight:700,color:"#1e293b"}}>{v}</p></div>
                      </div>
                    ))}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:12,marginBottom:16}}>
                    <div>
                      <p style={{fontSize:12,fontWeight:700,color:"#312e81",marginBottom:8}}>📊 난이도 분포</p>
                      {[["상",analysis.difficultyDistribution?.high||0,"#ef4444"],["중",analysis.difficultyDistribution?.medium||0,"#f59e0b"],["하",analysis.difficultyDistribution?.low||0,"#10b981"]].map(([l,v,c])=>(
                        <div key={l} style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                          <span style={{fontSize:10,fontWeight:700,color:c,width:14}}>{l}</span>
                          <div style={{flex:1,background:"#f1f5f9",borderRadius:20,height:8}}><div style={{width:`${v}%`,height:"100%",background:c,borderRadius:20}}/></div>
                          <span style={{fontSize:10,color:"#64748b",width:28}}>{v}%</span>
                        </div>
                      ))}
                    </div>
                    <div>
                      <p style={{fontSize:12,fontWeight:700,color:"#312e81",marginBottom:8}}>📝 출제 경향</p>
                      <div style={{background:"#f8fafc",padding:10,borderRadius:8,fontSize:10,color:"#475569",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{analysis.trends}</div>
                    </div>
                  </div>
                  <p style={{fontSize:12,fontWeight:700,color:"#312e81",marginBottom:8}}>✅ 문항별 분석</p>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}>
                    <thead><tr style={{background:"#f8fafc"}}>{["No","개념","난이도","결과","코멘트"].map(h=><th key={h} style={{padding:"6px 8px",textAlign:"left",fontWeight:700,color:"#64748b"}}>{h}</th>)}</tr></thead>
                    <tbody>{(analysis.questionAnalysis||[]).map((q,i)=>(
                      <tr key={i} style={{borderBottom:"1px solid #f1f5f9"}}>
                        <td style={{padding:"5px 8px",fontWeight:700,color:"#4f46e5"}}>{q.number}</td>
                        <td style={{padding:"5px 8px"}}>{q.topic}</td>
                        <td style={{padding:"5px 8px"}}><span style={{padding:"1px 5px",borderRadius:3,fontSize:9,fontWeight:700,background:q.difficulty==="상"?"#fee2e2":q.difficulty==="중"?"#fef3c7":"#d1fae5",color:q.difficulty==="상"?"#b91c1c":q.difficulty==="중"?"#92400e":"#065f46"}}>{q.difficulty}</span></td>
                        <td style={{padding:"5px 8px",textAlign:"center",fontWeight:700,color:q.isCorrect?"#059669":"#ef4444"}}>{q.isCorrect?"✓":"✗"}</td>
                        <td style={{padding:"5px 8px",color:"#64748b",fontSize:9}}>{q.comment}</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
                <div style={{padding:"32px 28px",borderTop:"3px solid #e2e8f0"}}>
                  <div style={{height:4,background:"#4f46e5",marginBottom:20}}/>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:20,paddingBottom:12,borderBottom:"2px solid #4f46e5"}}>
                    <div><h1 style={{fontSize:22,fontWeight:700,color:"#312e81"}}>시험분석 보고서</h1><span style={{background:"#4f46e5",color:"#fff",fontSize:10,padding:"2px 8px",borderRadius:4}}>PAGE 2/2</span></div>
                    <p style={{fontSize:11,color:"#94a3b8"}}>{new Date().toLocaleDateString()}</p>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                    <div style={{background:"#ecfdf5",padding:16,borderRadius:10,border:"1px solid #a7f3d0"}}><p style={{fontSize:12,fontWeight:700,color:"#065f46",marginBottom:8}}>✅ 강점</p><p style={{fontSize:10,color:"#334155",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{analysis.strengths}</p></div>
                    <div style={{background:"#fff1f2",padding:16,borderRadius:10,border:"1px solid #fecdd3"}}><p style={{fontSize:12,fontWeight:700,color:"#9f1239",marginBottom:8}}>⚠️ 약점</p><p style={{fontSize:10,color:"#334155",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{analysis.weaknesses}</p></div>
                  </div>
                  <div style={{background:"#1e1b4b",padding:20,borderRadius:10,marginBottom:16,borderTop:"5px solid #6366f1"}}>
                    <p style={{fontSize:12,fontWeight:700,color:"#a5b4fc",marginBottom:8}}>📅 NEXT LEVEL CURRICULUM</p>
                    {isEditing?<textarea value={editCurriculum} onChange={e=>setEditCurriculum(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.1)",border:"1px solid #6366f1",borderRadius:6,padding:8,color:"#e0e7ff",fontSize:10,resize:"vertical",minHeight:80}}/>:<p style={{fontSize:10,color:"#c7d2fe",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{editCurriculum}</p>}
                  </div>
                  <div style={{background:"#fff",padding:16,borderRadius:10,border:"2px solid #e2e8f0"}}>
                    <p style={{fontSize:10,fontWeight:700,color:"#4f46e5",marginBottom:8}}>💬 COACH MESSAGE</p>
                    {isEditing?<textarea value={editComments} onChange={e=>setEditComments(e.target.value)} style={{width:"100%",background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:6,padding:8,color:"#475569",fontSize:10,resize:"vertical",minHeight:60}}/>:<p style={{fontSize:10,color:"#475569",fontStyle:"italic",lineHeight:1.6}}>"{editComments}"</p>}
                  </div>
                </div>
              </div>
            </div>
...
function ProgressTab({coaches,addCoach,removeCoach}){
  ...
  async function handleAdd(){
    ...
    if(!n)return;
    if(coaches.includes(n)){alert("이미 등록된 코치입니다.");return;}
    await addCoach(n);setNewName("");
  }
        ...
        return(
          <div key={coach} style={{background:"#f9f9f9",borderRadius:10,border:"1px solid #eee",padding:"12px 14px",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:12,padding:"3px 10px",borderRadius:20,background:pct===100?"#E8F5E9":pct>0?"#E3F2FD":"#f5f5f5",color:pct===100?"#2E7D32":pct>0?"#1565C0":"#aaa",fontWeight:600}}>{done}/{TASKS.length} {pct===100?"✓ 완료":pct>0?"진행중":"미시작"}</span>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {TASKS.map(t=>(
                <span key={t.id} style={{fontSize:10,padding:"3px 8px",borderRadius:10,background:ck[t.id]?"#E8F5E9":"#fff",color:ck[t.id]?"#2E7D32":"#aaa",border:`1px solid ${ck[t.id]?"#A5D6A7":"#eee"}`}}>{ck[t.id]?"✓ ":""}{t.title}</span>
              ))}
            </div>
          </div>
        );
...
function FreeLessonNotice(){
  ...
  async function downloadJpg(){
    ...
    try{
      ...
      const a=document.createElement('a');a.href=canvas.toDataURL('image/jpeg',0.95);a.download=`무료수업안내_${form.month}월${form.day}일_${form.studentName||'학생'}.jpg`;a.click();
  ...
  const iSt={width:"100%",padding:"8px 10px",borderRadius:8,border:"1px solid #ddd",fontSize:13,marginBottom:0,background:"#fafafa"};
  return(
    <div className="card">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{background:"#FFFDF0",borderRadius:10,padding:14,border:"1px solid #FFE082"}}>
            <p style={{fontSize:13,fontWeight:600,color:"#B45309",marginBottom:10}}>ℹ️ 기본 정보</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <div style={{background:"#FFFDF0",borderRadius:10,padding:14,border:"1px solid #FFE082"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              <label><span style={lb}>오전/오후</span><select style={iSt} value={form.meridiem} onChange={e=>sf('meridiem',e.target.value)}>{MERIDIEMS.map(me=><option key={me}>{me}</option>)}</select></label>
          ...
          <div style={{background:"#FFFDF0",borderRadius:10,padding:14,border:"1px solid #FFE082"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              <label><span style={lb}>아이디</span><input style={iSt} value={form.testId} onChange={e=>sf('testId',e.target.value)} placeholder="ID"/></label>
              <label><span style={lb}>비밀번호</span><input style={iSt} value={form.testPassword} onChange={e=>sf('testPassword',e.target.value)} placeholder="PW"/></label>
            </div>
            <span style={lb}>안내 이미지</span>
            <div onClick={()=>fileRef.current?.click()} style={{border:"2px dashed #ddd",borderRadius:8,padding:14,textAlign:"center",cursor:"pointer",background:"#fafafa"}}>
              {form.guideImage?<div style={{position:"relative"}}><img src={form.guideImage} alt="guide" style={{width:"100%",borderRadius:6,maxHeight:150,objectFit:"contain"}}/><button onClick={e=>{e.stopPropagation();sf('guideImage','');}} style={{position:"absolute",top:4,right:4,background:"rgba(255,255,255,0.9)",border:"none",borderRadius:"50%",width:22,height:22,cursor:"pointer",color:"#EF5350",fontWeight:700}}>×</button></div>:<div style={{color:"#aaa",fontSize:12}}><div style={{fontSize:20,marginBottom:4}}>⬆</div><p>이미지 업로드</p></div>}
              <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={handleImg}/>
            </div>
          </div>
          <button onClick={downloadJpg} style={{padding:10,borderRadius:10,background:"#F59E0B",color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontSize:14}}>⬇ JPG 저장</button>
        </div>
        <div>
          <p style={{fontSize:13,fontWeight:600,color:"#333",marginBottom:8}}>👁 미리보기</p>
          <div style={{background:"#e5e7eb",borderRadius:16,padding:12,overflowY:"auto",maxHeight:600}}>
            <div ref={previewRef} style={{background:"#fff",padding:"24px 20px",position:"relative"}}>
              <div style={{position:"absolute",top:0,left:0,width:"100%",height:5,background:"#F59E0B"}}/>
              <div style={{marginTop:12,marginBottom:16}}>
                <p style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>안녕하세요. <span style={{color:"#B45309"}}>{form.studentName||"[학생이름]"}</span> 학생, <span style={{color:"#B45309"}}>{form.teacherName||"[선생님성함]"}</span> 코치입니다. 무료수업 확정 안내 드립니다.</p>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,padding:"12px 0",borderTop:"1px solid #FEF3C7",borderBottom:"1px solid #FEF3C7",marginBottom:12}}>
                <div><p style={{fontSize:12,fontWeight:700,color:"#B45309"}}>📅 {fmtDate}</p><p style={{fontSize:12,fontWeight:700,color:"#B45309",marginTop:4}}>🕐 {fmtTime}</p></div>
                <div style={{paddingLeft:10,borderLeft:"1px solid #FEF3C7"}}><p style={{fontSize:12,fontWeight:700,color:"#B45309"}}>👤 ID: <span style={{color:"#1565C0"}}>{form.testId||"미입력"}</span></p><p style={{fontSize:12,fontWeight:700,color:"#B45309",marginTop:4}}>🔒 PW: <span style={{color:"#1565C0"}}>{form.testPassword||"미입력"}</span></p></div>
              </div>
              {form.guideImage?<img src={form.guideImage} alt="guide" style={{width:"100%",borderRadius:6,marginBottom:12}}/>:<div style={{border:"2px dashed #FEF3C7",borderRadius:6,padding:"30px 0",textAlign:"center",color:"#FCD34D",marginBottom:12,fontSize:12}}>안내 이미지 영역</div>}
              <div style={{textAlign:"center"}}><p style={{fontSize:13,fontWeight:700}}>{form.teacherName?`${form.teacherName} 코치`:"코치"} 드림</p></div>
            </div>
          </div>
        </div>
      </div>
      ...
      {!preview?(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <label style={lb}><span style={{fontSize:12,color:"#555"}}>주 수업 횟수</span><select value={form.times} onChange={e=>set("times",e.target.value)} style={iSt2}>{[1,2,3,4,5,6,7,8].map(n=><option key={n} value={String(n)}>{n}타임</option>)}</select></label>
            <label style={lb}><span style={{fontSize:12,color:"#555"}}>1회 수업 시간(분)</span><input value={form.duration} onChange={e=>set("duration",e.target.value)} placeholder="60"/></label>
            ...
            <div style={{background:"#f0f7ff",borderRadius:10,padding:"12px 16px",border:"1px solid #BBDEFB",gridColumn:"1/-1",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><p style={{fontSize:11,color:"#1565C0",fontWeight:600,marginBottom:2}}>📊 예상 월 수업료</p><p style={{fontSize:11,color:"#888"}}>{form.type} · {form.schoolLevel} · {form.times}타임{form.planning?" + 플래닝":""}</p></div>
              <div style={{textAlign:"right"}}><p style={{fontSize:22,fontWeight:700,color:"#1565C0"}}>{calcFee()}원</p><input value={form.fee} onChange={e=>set("fee",e.target.value)} placeholder="직접 입력" style={{fontSize:11,padding:"4px 8px",marginBottom:0,marginTop:4,textAlign:"right",width:130}}/></div>
            </div>
                ...
                <div key={i} style={{border:"1px solid #eee",borderRadius:8,overflow:"hidden",marginBottom:8}}>
                  {editingMsg===i?(
                    <div style={{padding:"10px",background:"#f9f9f9"}}>
                      <input value={msg.title} onChange={e=>{const nx=[...messages];nx[i]={...nx[i],title:e.target.value};setMessages(nx);}} style={{marginBottom:6}}/>
                      <textarea value={msg.desc} onChange={e=>{const nx=[...messages];nx[i]={...nx[i],desc:e.target.value};setMessages(nx);}} rows={3} style={{resize:"vertical",marginBottom:6}}/>
                      <div style={{display:"flex",gap:6}}>
                        <button onClick={()=>setEditingMsg(null)} style={{...sBt,flex:1,padding:"6px"}}>완료</button>
                        <button onClick={()=>{const nx=[...messages];nx[i]={...DEFAULT_MESSAGES[i]};setMessages(nx);setEditingMsg(null);}} style={{...cBt,flex:1,padding:"6px",fontSize:11}}>초기화</button>
                      </div>
                    </div>
                  ):(
                    <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",cursor:"pointer"}} onClick={()=>setEditingMsg(i)}>
                      ...
                      <div style={{flex:1,minWidth:0}}><p style={{fontSize:12,fontWeight:600,color:"#333",marginBottom:2}}>{msg.title}</p><p style={{fontSize:11,color:"#888",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{msg.desc}</p></div>
                      <span style={{fontSize:11,color:"#5C6BC0",flexShrink:0}}>✏️</span>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div>
          ...
          <div ref={p1} style={{background:"#fff",padding:"32px 28px",fontFamily:"sans-serif",maxWidth:680,margin:"0 auto",border:"1px solid #ddd",borderRadius:8,marginBottom:16}}>
            <div style={{textAlign:"center",marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700,marginBottom:4}}>수업 일정 및 로그인 안내</h1><p style={{fontSize:12,color:"#888"}}>수업 일정, 수업료, 로그인 정보를 확인해주세요.</p></div>
            <div style={{border:"1.5px solid #4A90D9",borderRadius:10,padding:"16px 20px",marginBottom:16}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12}}><span>📅</span><h2 style={{fontSize:15,fontWeight:700}}>수업 일정 및 수업료</h2></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr"}}>
                <div style={{display:"flex",flexDirection:"column",gap:8,paddingRight:16,borderRight:"1px solid #eee"}}>
                  {[["회원 이름",form.name||"—"],["수업 형태",`${form.type} 수업 (${form.subject})`],["수업 시간",`1회 ${form.duration}분`],["수업 요일",form.days||"—"],["첫 수업",form.firstDate||"—"]].map(([k,v])=>(
                    <div key={k} style={{display:"flex",gap:10,fontSize:12}}><span style={{color:"#888",minWidth:64,flexShrink:0}}>{k}</span><span style={{color:k==="첫 수업"?"#1565C0":"#1a1a1a",fontWeight:k==="첫 수업"?600:400}}>{v}</span></div>
                  ))}
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:8,paddingLeft:16}}>
                  {[["수업 대상",form.grade||"—"],["수업 횟수",`주 ${form.times}회`]].map(([k,v])=>(
                    <div key={k} style={{display:"flex",gap:10,fontSize:12}}><span style={{color:"#888",minWidth:64,flexShrink:0}}>{k}</span><span style={{fontWeight:600,color:"#1565C0"}}>{v}</span></div>
                  ))}
                  <div style={{marginTop:6,padding:"10px 12px",background:"#f0f7ff",borderRadius:8,textAlign:"right"}}>
                    <p style={{fontSize:10,color:"#888",marginBottom:2}}>월 수업료</p>
                    <p style={{fontSize:20,fontWeight:700,color:"#1565C0"}}>{fee}원</p>
                  </div>
                </div>
              </div>
            </div>
            <div style={{border:"1.5px solid #4A90D9",borderRadius:10,padding:"16px 20px"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12}}><span>🛡</span><h2 style={{fontSize:15,fontWeight:700}}>학생 로그인 정보</h2></div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                {[["홈페이지","www.sscoaching.co.kr"],["학생 아이디",form.studentId||"미입력"],["학생 비밀번호",form.studentPw||"미입력"]].map(([k,v])=>(
                  <div key={k} style={{padding:"10px",background:"#f9f9f9",borderRadius:8,textAlign:"center"}}><p style={{fontSize:10,color:"#888",marginBottom:4}}>{k}</p><p style={{fontSize:12,fontWeight:600}}>{v}</p></div>
                ))}
              </div>
            </div>
          </div>
          <div ref={p2} style={{background:"#fff",padding:"32px 28px",fontFamily:"sans-serif",maxWidth:680,margin:"0 auto 16px",border:"1px solid #ddd",borderRadius:8}}>
            <div style={{textAlign:"center",marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700,marginBottom:4}}>학부모님께 드리는 말씀</h1><p style={{fontSize:12,color:"#888"}}>원활한 수업을 위해 꼭 확인 부탁드립니다.</p></div>
            {messages.map((item,i)=>(
              <div key={i} style={{display:"flex",gap:14,marginBottom:16,paddingBottom:16,borderBottom:"1px solid #f0f0f0"}}>
                <div style={{width:26,height:26,borderRadius:"50%",background:"#1565C0",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,flexShrink:0,marginTop:2}}>{i+1}</div>
                <div><p style={{fontSize:13,fontWeight:700,marginBottom:4}}>{item.title}</p><p style={{fontSize:11,color:"#555",lineHeight:1.7}}>{item.desc}</p></div>
              </div>
            ))}
          ...
          <div ref={p3} style={{background:"#fff",padding:"32px 28px",fontFamily:"sans-serif",maxWidth:680,margin:"0 auto",border:"1px solid #ddd",borderRadius:8}}>
            <div style={{textAlign:"center",marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700}}>결제 수단 및 방식 안내</h1></div>
            <p style={{fontSize:13,fontWeight:700,marginBottom:10}}>● <span style={{color:"#1565C0"}}>정기 결제 (카드)</span>: 매달 수업회차 완료 후 결제 예정문자 발송, 2일 후 자동 결제됩니다.</p>
            <div style={{border:"1px solid #ddd",borderRadius:10,overflow:"hidden",marginBottom:20}}>
              <div style={{background:"#f5f8ff",padding:"8px 14px",borderBottom:"1px solid #eee",fontSize:12,color:"#555"}}>마이페이지 → 결제 → 회비 정기결제</div>
              <div style={{padding:"14px",fontSize:12,color:"#555"}}>정기결제 서비스 이용 동의 체크 후 → 정기결제 신청 버튼 클릭</div>
            </div>
            <p style={{fontSize:13,fontWeight:700,marginBottom:10}}>● <span style={{color:"#1565C0"}}>알림톡 결제</span>: 카카오톡 알림톡으로 발송된 결제 링크를 클릭하여 간편 결제 가능합니다.</p>
          </div>
        </div>
      )}
    </div>
  ...
  if(loading)return(
    <>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f0f2ff"}}>
        <div style={{textAlign:"center",color:"#5C6BC0"}}><div style={{width:40,height:40,border:"4px solid #c5cae9",borderTopColor:"#5C6BC0",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 12px"}}/><p style={{fontSize:14}}>불러오는 중...</p></div>
      </div>
    </>
  );
  ...
  return(
    <>
      ...
      <div className="portal">
        ...
        <div className="tabs">
          {[["calendar","📅 캘린더"],["resource","📂 자료실"],["settle","💰 정착지원금"],["notice","📌 공지사항"],["notice_gen","📄 수업안내문"],["free_lesson","🆓 무료수업"],["exam_analysis","📊 시험분석"],...(isAdmin?[["progress","👥 코치현황"]]:[])]
            ...
            <div style={{overflowX:"auto"}}>
              <table className="settle-table">
                ...
                <tbody>
                  ...
                  <tr><td rowSpan={5} style={{background:"#F3E5F5",color:"#6A1B9A"}}>300,000원</td><td rowSpan={5} style={{fontSize:10,color:"#888"}}>× 계약이행일수<br/>÷ 전체일수</td><td>0T초과~10T미만</td><td style={{color:"#D32F2F",fontWeight:600}}>+100,000원</td></tr>
